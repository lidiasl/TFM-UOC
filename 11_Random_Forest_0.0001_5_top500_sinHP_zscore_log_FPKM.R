#####################################################################
# Genes of pvalue adj < 0.0001 & log2FC>5 & top 500 in each tumor
# zscore_log_FPKM
# Lidia Sainz
#####################################################################
...
for (i in 1:length(names_enfermedad_primaria)){
  ...
  list_top500[[i]] <- rownames(zscore_log_fpkm.var)
}

names(list_top500) <- names_enfermedad_primaria

list_top500 <- Reduce(c,list_top500)
list_top500 <- unique(list_top500) #1521


zscore_log_fpkm.var <- zscore_log_fpkm[rownames(zscore_log_fpkm)%in%list_top500,]
...
tic-toc #Time difference of 8.63961 hours
