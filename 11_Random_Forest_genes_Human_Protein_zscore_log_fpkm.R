...
#### RANDOM FOREST
######################################
zscore_log_fpkm.f$enfermedad_primaria <- as.factor(zscore_log_fpkm.f$enfermedad_primaria)

####
tunegrid <- expand.grid(mtry=seq(35,285,25))
...
tic-toc #Time difference of 7.346589 hours
...
write.csv(bestTune_df, file="bestTune_df_Human_Protein_zscore_log_FPKM.csv")
...
set.seed(42)
model_rf <- randomForest(x = as.matrix(zscore_log_fpkm.f[, colnames(zscore_log_fpkm.f)!="enfermedad_primaria"]),
                         y=zscore_log_fpkm.f$enfermedad_primaria,
                         mtry=135,
                         ntree=1500,
                         keep.forest = TRUE,
                         importance = TRUE)
predRF <- predict(model_rf)
tab_cm <- confusionMatrix(predRF,zscore_log_fpkm.f$enfermedad_primaria)
#Accuracy : 0.9539
#Kappa : 0.9522
...
######################################
setwd(resultsDirectory)
saveRDS(tab_cm, file="tab_cm_train_RF_Human_Protein_zscore_log_FPKM.rds")
...
zscore_log_FPKM_test <- zscore_log_FPKM_test[rownames(zscore_log_FPKM_test) %in% genes_intersect,]
zscore_log_FPKM_test <- as.data.frame(t(zscore_log_FPKM_test))

table(colnames(zscore_log_FPKM_test)==colnames(zscore_log_fpkm.f)[colnames(zscore_log_fpkm.f)!="enfermedad_primaria"])
...
######################################
setwd(resultsDirectory)
saveRDS(tab_cm_VAL, file="tab_cm_VAL_RF_Human_Protein_zscore_log_FPKM.rds")
...
zscore_log_FPKM_MET500 <- zscore_log_FPKM_MET500[rownames(zscore_log_FPKM_MET500) %in% genes_intersect,]
zscore_log_FPKM_MET500 <- as.data.frame(t(zscore_log_FPKM_MET500))

table(colnames(zscore_log_FPKM_MET500)==colnames(zscore_log_fpkm.f)[colnames(zscore_log_fpkm.f)!="enfermedad_primaria"])
zscore_log_FPKM_MET500 <- zscore_log_FPKM_MET500[, match(colnames(zscore_log_fpkm.f)[colnames(zscore_log_fpkm.f)!="enfermedad_primaria"], colnames(zscore_log_FPKM_MET500))]
...
######################################
setwd(resultsDirectory)
saveRDS(tab_cm_MET500, file="tab_cm_MET500_RF_Human_Protein_zscore_log_FPKM.rds")
