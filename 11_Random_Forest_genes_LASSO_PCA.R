...
genes_LASSO <- read.csv(file="nonzero_genes_LASSO.csv")
...
######################################
zscore_variable_log_fpkm.f <- zscore_variable_log_fpkm[rownames(zscore_variable_log_fpkm) %in% genes_LASSO,]
...
set.seed(42)
PCA <- prcomp(as.matrix(zscore_variable_log_fpkm.f[,colnames(zscore_variable_log_fpkm.f)!="enfermedad_primaria"]), 
              scale. = FALSE, 
              center = FALSE)
...
tic-toc #Time difference of 1.618229 hours
...
write.csv(bestTune_df, file="bestTune_df_LASSO_PCA.csv")
...
zscore_variable_log_FPKM_test <- zscore_variable_log_FPKM_test[rownames(zscore_variable_log_FPKM_test) %in% genes_LASSO,]
...
zscore_variable_log_FPKM_MET500 <- zscore_variable_log_FPKM_MET500[rownames(zscore_variable_log_FPKM_MET500) %in% genes_LASSO,]
