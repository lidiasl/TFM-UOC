#####################################################################
# Genes of pvalue adj < 0.0001 & log2FC>5 & top 300 in each tumor
# zscore_log_FPKM
# Lidia Sainz
#####################################################################
...
for (i in 1:length(names_enfermedad_primaria)){
  ...
  zscore_log_fpkm.var <- zscore_log_fpkm.f[order(rowVars(as.matrix(zscore_log_fpkm.f)), decreasing = TRUE)[1:300],]
  list_top300[[i]] <- rownames(zscore_log_fpkm.var)
}

names(list_top300) <- names_enfermedad_primaria

list_top300 <- Reduce(c,list_top300)
list_top300 <- unique(list_top300) #912


zscore_log_fpkm.var <- zscore_log_fpkm[rownames(zscore_log_fpkm)%in%list_top300,]
...
####
tunegrid <- expand.grid(mtry=seq(20,100,10))
...
tic-toc #Time difference of 3.016116 hours
...
write.csv(bestTune_df, file="bestTune_df_0.0001_5_top300_zscore_log_FPKM.csv")
