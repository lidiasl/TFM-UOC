#####################################################################
# Genes of pvalue adj < 0.0001 & log2FC>3 & top 1000 in SVM by gene
# zscore_log_FPKM
# Lidia Sainz
#####################################################################
...
tic-toc #Time difference of 23.61369 mins
...
tab_cm <- confusionMatrix(predSVM,zscore_variable_log_fpkm.var$enfermedad_primaria)
######################################
setwd(resultsDirectory)
write.csv(bestTune_list, file="bestTune_SVM_0.0001_5_top1000_zscore_variable_log_FPKM.csv")

#### 

setwd(resultsDirectory)
save(model_SVM, file = "model_SVM_variable.RData")
#Accuracy : 1
#Kappa : 1   
...
pred_val <- predict(model_SVM, zscore_variable_log_FPKM_test)
...
predRF_MET500 <- predict(model_SVM, zscore_variable_log_FPKM_MET500)
...
######################################
setwd(resultsDirectory)
saveRDS(tab_cm_MET500, file="tab_cm_MET500_SVM_top1000_zscore_variable_log_FPKM.rds")
...
setwd(resultsDirectory)
write.csv(df_byClass_training, file="df_byClass_training_SVM_variable.csv")
write.csv(df_byClass_VAL, file="df_byClass_VAL_SVM_variable.csv")
write.csv(df_byClass_MET500, file="df_byClass_MET500_SVM_variable.csv")
