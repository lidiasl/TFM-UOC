...
resultsDirectory <- "~/Desktop/LIDIA/TCGA_classification/TFM/Results/PCA genes"
...
set.seed(42)
PCA <- prcomp(as.matrix(zscore_variable_log_fpkm[,colnames(zscore_variable_log_fpkm)!="enfermedad_primaria"]), 
              scale. = FALSE, 
              center = FALSE)

library(factoextra)
fviz_eig(PCA, addlabels = TRUE, labelsize=0.1,ncp=65)+ 
...
saveRDS(transformacion_PCA, file="transformacion_PCA.csv")
######################################
...
PCA_training <- PCA_training[, 1:63]
...
####
tunegrid <- expand.grid(mtry=seq(5,18,3))
...
tic-toc #Time difference of 16.47736 mins
...
write.csv(bestTune_df, file="bestTune_df_PCA.csv")
...
#65 components, mtry=5, ntree=1300, padj<0.001 log2FC>3
#Accuracy : 0.9697
#Kappa : 0.9686

#63 components, mtry=5, ntree=1000, padj<0.0001 log2FC>5
...
######################################
setwd("~/Desktop/LIDIA/TCGA_classification/TFM/Results/Random Forest Prueba")
saveRDS(tab_cm, file="tab_cm_train_RF_PCA.rds")
...
table(colnames(zscore_variable_log_FPKM_test)==rownames(transformacion_PCA))

zscore_log_FPKM_test_PCA <- as.matrix(zscore_variable_log_FPKM_test) %*% transformacion_PCA
zscore_log_FPKM_test_PCA <- as.data.frame(zscore_log_FPKM_test_PCA)
zscore_log_FPKM_test_PCA <- zscore_log_FPKM_test_PCA[, 1:63]
...
######################################
setwd("~/Desktop/LIDIA/TCGA_classification/TFM/Results/Random Forest Prueba")
saveRDS(tab_cm_VAL, file="tab_cm_VAL_RF_PCA.rds")
...
table(colnames(zscore_variable_log_FPKM_MET500)==rownames(transformacion_PCA))
table(colnames(zscore_variable_log_FPKM_MET500) %in% rownames(transformacion_PCA))
zscore_variable_log_FPKM_MET500 <- zscore_variable_log_FPKM_MET500[, match(rownames(transformacion_PCA), colnames(zscore_variable_log_FPKM_MET500))]

zscore_log_FPKM_MET500_PCA <- as.matrix(zscore_variable_log_FPKM_MET500) %*% transformacion_PCA
zscore_log_FPKM_MET500_PCA <- as.data.frame(zscore_log_FPKM_MET500_PCA)
zscore_log_FPKM_MET500_PCA <- zscore_log_FPKM_MET500_PCA[, 1:63]
...
######################################
setwd("~/Desktop/LIDIA/TCGA_classification/TFM/Results/Random Forest Prueba")
saveRDS(tab_cm_MET500, file="tab_cm_MET500_RF_PCA.rds")
