#####################################################################
# Genes of pvalue adj < 0.0001 & log2FC>5 & top 100 in each tumor
# zscore_log_FPKM
# Lidia Sainz
#####################################################################
...
for (i in 1:length(names_enfermedad_primaria)){
  ...
  zscore_log_fpkm.var <- zscore_log_fpkm.f[order(rowVars(as.matrix(zscore_log_fpkm.f)), decreasing = TRUE)[1:100],]
  list_top100[[i]] <- rownames(zscore_log_fpkm.var)
}

names(list_top100) <- names_enfermedad_primaria

list_top100 <- Reduce(c,list_top100)
list_top100 <- unique(list_top100) #289


zscore_log_fpkm.var <- zscore_log_fpkm[rownames(zscore_log_fpkm)%in%list_top100,]
...
####
tunegrid <- expand.grid(mtry=seq(5,70,5))
...
tic-toc #Time difference of 1.250957 hours
...
write.csv(bestTune_df, file="bestTune_df_0.0001_5_top100_zscore_log_FPKM.csv")
